...
def process_song_data(spark, input_data, output_data):
    """
    Load song data from S3 location and store as parquet partitioned by year and artist_id.
...
def process_log_data(spark, input_data, output_data):
    """
    Load log event data from S3 location,
    Create songplays, users and time tables and store as parquet partitioned by year and month.
    ...
    time_df = time_df.select(time_df.start_time, time_df.hour, time_df.day, time_df.week, time_df.month, time_df.year, time_df.weekday).sort("start_time")
